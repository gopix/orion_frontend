...
const SAMPLE_GUIDELINES = [
  { id: 1, title: "Chapter Length & Structure", content: "Each chapter should be 2,500–4,000 words with a summary box.", categoryId: 1, categoryName: "Exam Preparation", status: "Active", createdAt: "2026-06-03T10:00:00Z" },
  { id: 2, title: "Citation Style", content: "Use APA 7th edition for all references.", categoryId: 1, categoryName: "Exam Preparation", status: "Active", createdAt: "2026-06-05T10:00:00Z" },
  { id: 3, title: "Reading Level", content: "Target Flesch-Kincaid grade level 6–8.", categoryId: 2, categoryName: "K-12 Curriculum", status: "Active", createdAt: "2026-06-12T10:00:00Z" },
];
...
// Active/Inactive status chips used on Category & Guidelines lists.
const activeStatusChipClass = (status) =>
  (status || "").toLowerCase() === "active" ? "bf-chip-complete" : "bf-chip-draft";
...
export default function Submit() {
  ...
  /* ── Guidelines (USER role only) ──────────────────────────────
     ...
     Category (category_id from the real Category list above).
     Sample local state until the Guidelines endpoint is given. */
  const [guidelineSubTab, setGuidelineSubTab] = useState("new-guideline");
  const [guidelineList, setGuidelineList] = useState(SAMPLE_GUIDELINES);
  const [guidelineForm, setGuidelineForm] = useState({ categoryId: "", title: "", content: "", status: "Active" });
  const [guidelineSaving, setGuidelineSaving] = useState(false);
  const [guidelineSuccess, setGuidelineSuccess] = useState("");

  const handleCreateGuideline = (e) => {
    e.preventDefault();
    if (!guidelineForm.title.trim() || !guidelineForm.categoryId) return;
    setGuidelineSaving(true);
    // TODO: replace with real API call (e.g. POST /book-forge/guidelines)
    setTimeout(() => {
      const category = categoryList.find((c) => String(c.category_id) === String(guidelineForm.categoryId));
      setGuidelineList((prev) => [
        {
      ...
      setGuidelineForm({ categoryId: "", title: "", content: "", status: "Active" });
      setGuidelineSaving(false);
      setGuidelineSuccess("Guideline created successfully.");
      setGuidelineSubTab("list-guideline");
      setTimeout(() => setGuidelineSuccess(""), 4000);
    }, 500);
  };
                  ...
                  <form onSubmit={handleCreateGuideline}>
                    <div className="bf-form-row">
                      <div className="bf-form-group bf-form-group-full">
                        ...
                        <select
                          className="bf-form-input"
                          value={guidelineForm.categoryId}
                          onChange={(e) => setGuidelineForm({ ...guidelineForm, categoryId: e.target.value })}
                        >
                    ...
                    <div className="bf-form-row">
                      <div className="bf-form-group bf-form-group-full">
                        <label className="bf-form-label">Guideline Title *</label>
                        <input
                          type="text"
                          className="bf-form-input"
                          placeholder="e.g. Chapter Length & Structure"
                    ...
                    <div className="bf-form-row">
                      <div className="bf-form-group bf-form-group-full">
                        <label className="bf-form-label">Guideline Content</label>
                        <textarea
                          className="bf-form-input"
                          rows={5}
                    ...
                    <div className="bf-form-row">
                      <div className="bf-form-group">
                        <label className="bf-form-label">Status</label>
                        <select
                          className="bf-form-input"
                          value={guidelineForm.status}
                          onChange={(e) => setGuidelineForm({ ...guidelineForm, status: e.target.value })}
                        >
                    ...
                    <div className="bf-step-actions">
                      <button
                        type="button"
                        className="bf-btn bf-btn-secondary"
                        onClick={() => setGuidelineForm({ categoryId: "", title: "", content: "", status: "Active" })}
                      >
                        Clear
                      </button>
                      <button
                        type="submit"
                        className="bf-btn bf-btn-primary"
                        disabled={!guidelineForm.title.trim() || !guidelineForm.categoryId || guidelineSaving}
                      >
                        {guidelineSaving ? "Saving…" : "Create Guideline"}
                      </button>
                    </div>
                  </form>
                )}
                      ...
                      <tbody>
                        {guidelineList.map((g) => (
                          <tr key={g.id}>
                            ...
                            <td><span className={`bf-chip ${activeStatusChipClass(g.status)}`}>{g.status}</span></td>
                            <td>{formatDocDate(g.createdAt)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
