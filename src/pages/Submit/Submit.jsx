...
export default function Submit() {
  ...
  /* ── Document metadata (nested modal) ─────────────────────── */
  const [metadataModal, setMetadataModal] = useState(null); // { loading, error, data } | null

  const handleViewMetadata = async (doc) => {
    setMetadataModal({ loading: true, error: "", data: null });
    try {
      const res = await getBookForgeDocumentMetadata(doc.id);
      if (res?.errors && res.errors.length > 0) {
        throw new Error(res.errors[0]?.message || "Failed to load document metadata.");
      }
      setMetadataModal({ loading: false, error: "", data: res?.data || doc });
    } catch (err) {
      console.error("Failed to load document metadata:", err);
      setMetadataModal({ loading: false, error: "Failed to load document metadata.", data: null });
    }
  };
  ...
  return (
    <div className="submit-page">
      ...
      {metadataModal && (
        <DocumentMetadataModal state={metadataModal} onClose={closeMetadataModal} />
      )}
    </div>
  );
}
    ...
    <div className="bf-modal-overlay" onClick={onClose}>
      <div className="bf-modal bf-modal-docs" onClick={(e) => e.stopPropagation()}>
        <div className="bf-modal-header">
          <h3>📚 Documents{project ? ` — ${project.title}` : ""}</h3>
...
function DocumentMetadataModal({ state, onClose }) {
  ...
  const rows = data
    ? [
        ...
        ["Stored Filename", data.stored_filename],
  ...
  return (
    <div className="bf-modal-overlay" onClick={onClose}>
      <div className="bf-modal bf-modal-metadata" onClick={(e) => e.stopPropagation()}>
        ...
        {loading ? (
          <p className="bf-doc-empty">Loading metadata…</p>
        ) : error ? (
