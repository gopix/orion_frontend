...
export default function Template() {
  ...
  // Orgs for direct-fetch dropdown
  const [orgFetchOrgs, setOrgFetchOrgs]               = useState([]);
  const [orgFetchOrgsLoading, setOrgFetchOrgsLoading] = useState(false);
  const [orgFetchSelectedName, setOrgFetchSelectedName] = useState("");
  ...
  const [bottomFetchOrgName, setBottomFetchOrgName]   = useState("");
  ...
  const goToOrgTab = async () => {
    ...
    setOrgFetchError("");
    setOrgEntries([]);
    setOrgFetchSelectedName("");
    setActiveNav("organization");
    // Load organizations for the dropdown
    setOrgFetchOrgsLoading(true);
    try {
      const data = await getOrganizations();
      if (Array.isArray(data))               setOrgFetchOrgs(data);
      else if (Array.isArray(data?.data))    setOrgFetchOrgs(data.data);
      else if (Array.isArray(data?.results)) setOrgFetchOrgs(data.results);
      else                                   setOrgFetchOrgs([]);
    } catch {
      setOrgFetchOrgs([]);
    } finally {
      setOrgFetchOrgsLoading(false);
    }
  };
  ...
  const handleOrgDirectFetch = async () => {
    if (!orgFetchOrgId.toString().trim()) return alert("Please select an Organization.");
    ...
    try {
      ...
      const mapped = entries.map((item, idx) => ({
        ...
        orgId: orgFetchOrgId,
        orgName: orgFetchSelectedName || (item.organization_name ?? item.orgName ?? `Org ${orgFetchOrgId}`),
  ...
  const handleBottomFetch = async () => {
    ...
    if (useOrg && !bottomFetchOrgId.toString().trim())   return alert("Please select an Organization.");
    ...
    try {
      ...
      let mapped = entries.map((item, idx) => ({
        ...
        id: item.id ?? `fetched_${idx}_${Date.now()}`,
        orgId: orgIdToFetch,
        orgName: (item.organization_name ?? item.orgName) ?? (bottomFetchOrgName || `Org ${orgIdToFetch}`),
    ...
    <div className="tp-page" onClick={() => {}}>
      ...
      <aside className="tp-sidebar">
        <nav className="tp-nav">
          ...
          {/* 1. Remediate */}
          <div
            className={`tp-nav-item tp-nav-parent ${activeNav === "remediate" ? "active" : ""}`}
            onClick={() => setActiveNav(activeNav === "remediate" ? "" : "remediate")}
          >
            <span className="tp-nav-icon">🛠</span> Remediate
            <span className="tp-chevron">{activeNav === "remediate" ? "▾" : "▸"}</span>
          </div>
          {activeNav === "remediate" && (
            <div className="tp-nav-sub">
              <div className="tp-nav-sub-item" onClick={() => navigate("/remediate-pdf")}>
          ...
          {(activeNav === "master" || activeNav === "organization") && (
            <div className="tp-nav-sub">
              <div className={`tp-nav-sub-item ${activeNav === "master" ? "active" : ""}`} onClick={() => setActiveNav("master")}>
                <span className="tp-sub-dot">›</span> Master
              </div>
              <div className={`tp-nav-sub-item ${activeNav === "organization" ? "active" : ""}`} onClick={goToOrgTab}>
                <span className="tp-sub-dot">›</span> Organization
              </div>
            </div>
          )}
        </nav>
        <div className="tp-sidebar-footer">
          <div className="tp-user-section">
            <p className="tp-user-email">{sessionStorage.getItem("userEmail")}</p>
            <button 
                ...
                <div className="tp-org-fetch-panel">
                  ...
                  <div className="tp-org-fetch-fields">
                    <div className="tp-org-fetch-field">
                      <label className="tp-label">Organization <span style={{ color: "#dc2626" }}>*</span></label>
                      {orgFetchOrgsLoading ? (
                        <p style={{ fontSize: 13, color: "#64748b" }}>⏳ Loading organizations…</p>
                      ) : (
                        <select
                          className="tp-input"
                          value={orgFetchOrgId}
                          onChange={(e) => {
                            setOrgFetchOrgId(e.target.value);
                            const picked = orgFetchOrgs.find(
                              (o) => String(o.id ?? o.organization_id ?? o.org_id) === e.target.value
                            );
                            setOrgFetchSelectedName(
                              picked
                                ? (picked.name ?? picked.organization_name ?? picked.org_name ?? `Org ${e.target.value}`)
                                : ""
                            );
                          }}
                        >
                          <option value="">— Select an organization —</option>
                          {orgFetchOrgs.map((org, idx) => {
              ...
              {!cameFromMaster && orgFetchDone && (
                <div style={{ marginBottom: 16 }}>
                  ...
                  <span style={{ marginLeft: 12, fontSize: 12, color: "#64748b" }}>
                    Org: <strong>{orgFetchSelectedName || orgFetchOrgId}</strong>
                  ...
                  {showOrgFetchPanel && (
                    <div className="tp-bottom-fetch-body">
                      ...
                      <div className="tp-bottom-fetch-checks">
                        ...
                        <div className={`tp-bottom-fetch-option ${fetchByOrgChecked ? "selected" : ""}`}>
                          <label className="tp-bottom-fetch-check-label">
                            ...
                            />
                            <div>
                              <span className="tp-bottom-fetch-option-title">🏢 By Organization</span>
                          ...
                          {fetchByOrgChecked && (
                            <select
                              ...
                              onChange={(e) => {
                                setBottomFetchOrgId(e.target.value);
                                const picked = organizations.find(
                                  (o) => String(o.id ?? o.organization_id ?? o.org_id) === e.target.value
                                );
                                setBottomFetchOrgName(
                                  picked
                                    ? (picked.name ?? picked.organization_name ?? picked.org_name ?? `Org ${e.target.value}`)
                                    : ""
                                );
                      ...
                      <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
                        <button className="tp-btn tp-btn-outline" onClick={() => { setShowOrgFetchPanel(false); setFetchByOrgChecked(false); setFetchByProjChecked(false); setBottomFetchOrgId(""); setBottomFetchOrgName(""); }}>
                          Cancel
                        </button>
                        <button
                          className="tp-btn tp-btn-primary"
