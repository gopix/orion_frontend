...
//                       Add Accessibility Checks Manually
//                     </h3>
//                     {/* ✕ cross — cancels the form and returns to master template view */}
//                     <button
...
export default function Template() {
  ...
  // Organizations loaded from GET /api/v1/master-setup/organizations
  const [organizations, setOrganizations]           = useState([]);
  const [orgsLoading, setOrgsLoading]               = useState(false);
  const [orgsError, setOrgsError]                   = useState("");
  const [selectedOrgId, setSelectedOrgId]           = useState(""); // org chosen in batch-clone modal
  ...
  const openBatchCloneModal = async () => {
    ...
    setOrgsError("");
    setShowBatchCloneModal(true);
    // Fetch organizations from GET /api/v1/master-setup/organizations
    setOrgsLoading(true);
    try {
        ...
        <div className="tp-modal-overlay" onClick={() => setShowBatchCloneModal(false)}>
          <div className="tp-modal tp-modal-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="tp-modal-body" style={{ padding: 0 }}>
              ...
              {/* ── Organization selector (from GET /api/v1/master-setup/organizations) ── */}
              <div className="tp-clone-org-selector">
                ...
                {orgsLoading && (
                  <p className="tp-clone-org-hint">⏳ Loading organizations…</p>
                )}
                {orgsError && (
                  <p className="tp-clone-org-hint" style={{ color: "#dc2626" }}>⚠ {orgsError}</p>
                )}
            ...
            <div className="tp-modal-footer">
              <button className="tp-btn tp-btn-outline" onClick={() => setShowBatchCloneModal(false)}>Cancel</button>
              <button className="tp-btn tp-btn-primary" onClick={confirmBatchClone} disabled={!batchSelected.length || !selectedOrgId}>
                ✅ Clone Selected ({batchSelected.length})
              </button>
            </div>
          </div>
